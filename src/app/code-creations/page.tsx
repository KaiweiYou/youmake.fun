...
import { getProjectsByCategory, CodeProject } from '@/data/projects'

export default function CodeCreations() {
  const codeProjects = getProjectsByCategory('code-creations') as CodeProject[];
          ...
          >
            ...
            <div className="relative h-48 w-full bg-gray-200 dark:bg-gray-800">
              ...
              <div className="absolute top-3 right-3 z-10">
                <span className={`px-2 py-1 text-xs font-medium rounded-full shadow-sm ${getStatusColor(project.status)}`}>
                  {project.status}
                </span>
              </div>
              {/* Category Badge */}
              <div className="absolute top-3 left-3 z-10">
                <span className="px-2 py-1 text-xs font-medium rounded-full bg-black/50 text-white backdrop-blur-sm border border-white/20">
                  {project.projectType}
                </span>
              </div>
            </div>
